...
public class HyperBall extends Vertex<IntWritable, FacilityLocationVertexValue, DoubleWritable, HyperLogLog>{
  ...
  public void compute(
	  ...
	  double numSteps = getValue().getRadius();
	  ...
	  if (getSuperstep() == 0L) {
		  ...
		  for (Edge<IntWritable, DoubleWritable> edge : getEdges()) {
			  IntWritable vertexId = edge.getTargetVertexId();
