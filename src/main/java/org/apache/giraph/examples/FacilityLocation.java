...
public class FacilityLocation extends Vertex<LongWritable,
  ...
  private Vertex<IntWritable, FacilityLocationVertexValue, IntWritable, IntPairWritable> vertex;
  ...
  public void compute(Iterable<IntPairWritable> messages) {
	  ...
	  if(state==state.HB) {
		  hyperBallComputation();
	  }
	  else if(state==state.FM) {
		  freezeMessageComputation();
	  }
	  */
	  
	  int maxDist = -1000000, maxId = 0, remainingDistance;
	  
	  if(getSuperstep()==0) {
		  for (Edge<LongWritable, IntWritable> edge : getEdges()) {
			  remainingDistance = (int) (alpha - edge.getValue().get());
			  if(remainingDistance>0) {
				  sendMessage(edge.getTargetVertexId(), new IntPairWritable((int)getId().get(),remainingDistance));
			  }
		  }
	  }
	  else {
		  for (IntPairWritable message: messages) {
			  ...
			  remainingDistance = message.getSecond();
			  if(id==getId().get())
				  continue;
			  for (Edge<LongWritable, IntWritable> edge : getEdges()) {
				  ...
				  if(remainingDistance>=0) {
					  ...
					  if(id!=edge.getTargetVertexId().get())
						  sendMessage(edge.getTargetVertexId(), new IntPairWritable(maxId,remainingDistance));
				  }
				  else {
					  voteToHalt();
				  }
